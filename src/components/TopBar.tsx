...
import { ChevronDown, Bell, CheckCircle2, AlertCircle, Info, X, Menu, Plus } from 'lucide-react';
import { useNavigate } from 'react-router-dom';
import { useAuth } from '../context/AuthContext';
import { mockBrandNotifications } from '../data/mock';
import { useBrandProfiles } from '../hooks/useBrandProfiles';

export default function TopBar() {
  const { user } = useAuth();
  const navigate = useNavigate();
  const { profiles, selectedProfile, selectProfile, loading } = useBrandProfiles();
  const [dropdownOpen, setDropdownOpen] = useState(false);
  const [notificationsOpen, setNotificationsOpen] = useState(false);
  const [notifications, setNotifications] = useState(mockBrandNotifications);
  ...
  const getIcon = (type: string) => {
    switch (type) {
      case 'submission': return <Info className="w-4 h-4 text-cyan" />;
      case 'campaign': return <CheckCircle2 className="w-4 h-4 text-green-400" />;
      case 'creator': return <AlertCircle className="w-4 h-4 text-purple-400" />;
      ...
      <div className="flex items-center gap-4">
        ...
        <div className="relative" ref={dropdownRef}>
          <button 
            ...
            className="flex items-center gap-2 md:gap-3 px-3 md:px-4 py-2 rounded-xl bg-white/5 border border-white/10 hover:bg-white/10 transition-colors shadow-[inset_0_1px_1px_rgba(255,255,255,0.1)]"
          >
            {selectedProfile ? (
              <>
                <img 
                  src={selectedProfile.logo_url || `https://ui-avatars.com/api/?name=${encodeURIComponent(selectedProfile.company_name)}&background=random`} 
            ...
            <div className="absolute top-full left-0 mt-2 w-56 glass-panel rounded-2xl overflow-hidden py-2 shadow-2xl origin-top-left animate-in fade-in zoom-in-95 duration-200">
              ...
              {profiles.length > 0 ? (
                profiles.map(profile => (
                  <button
                    key={profile.id}
                    onClick={() => { selectProfile(profile.id); setDropdownOpen(false); }}
              ...
              <div className="border-t border-white/10 mt-2 pt-2">
                <button
                  onClick={() => { navigate('/brand/profiles/new'); setDropdownOpen(false); }}
                  className="w-full flex items-center gap-3 px-4 py-3 hover:bg-white/5 transition-colors text-left text-cyan"
                >
                      ...
                      >
                        <div className="flex gap-3">
                          <div className={`mt-0.5 shrink-0 w-8 h-8 rounded-full flex items-center justify-center ${
                            notification.type === 'campaign' ? 'bg-green-500/10' :
                            notification.type === 'creator' ? 'bg-purple-500/10' :
                            'bg-cyan/10'
                          }`}>
