...
function getCampaignStatus(status: string) {
  if (status === 'live') return { label: 'Active', className: 'bg-cyan/10 text-cyan border-cyan/20' };
  if (status === 'completed') return { label: 'Completed', className: 'bg-green-500/10 text-green-400 border-green-500/20' };
  return { label: 'Draft', className: 'bg-white/10 text-muted border-white/20' };
}
...
export default function BrandDashboard() {
  ...
  return (
    <div className="min-h-screen bg-[#0A0A1E] text-[#F5F5F7] font-sans selection:bg-cyan/30 flex">
      <BrandSidebar /><TopBar />
      <main className="brand-page-main flex-1 md:ml-64 p-4 md:p-8"><div className="max-w-6xl mx-auto">
        ...
        <div className="mb-12 grid grid-cols-4 gap-1.5 md:grid-cols-2 md:gap-6 lg:grid-cols-4">{statCards.map((stat, index) => { const Icon = stat.icon; return <motion.div key={stat.title} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8, ease: appleEase, delay: 0.1 + index * 0.05 }} className="glass-panel group relative min-w-0 overflow-hidden rounded-[2rem] p-2 md:p-6"><div className="relative z-10 mb-2 flex flex-col items-start gap-2 md:mb-4 md:flex-row md:items-center md:justify-between"><span className="text-[0.6rem] font-medium leading-tight text-muted md:text-sm">{stat.title}</span><div className={`flex h-7 w-7 shrink-0 items-center justify-center rounded-2xl border border-white/10 md:h-12 md:w-12 ${stat.bg} ${stat.color}`}><Icon className="h-3.5 w-3.5 md:h-6 md:w-6" /></div></div><div className="relative z-10 break-words text-[0.9rem] font-bold leading-tight tracking-tight text-white md:text-3xl md:font-semibold">{stat.value}</div></motion.div>; })}</div>
