...
export default function BrandWallet() {
  ...
  const escrow = confirmedCampaigns
    ...
    .reduce((sum, c: any) => sum + Number(c.escrowed_budget || c.net_budget || 0), 0);
  const transactions = confirmedCampaigns.map((c: any) => ({
    id: c.id,
    date: c.created_at,
    campaign: c.title,
    amount: Number(c.budget || 0),
    txHash: c.escrow_tx_hash,
    txUrl: getTxUrl(c.escrow_tx_hash),
    type: 'Funded',
    status: 'Confirmed',
  }));
      ...
      <main className="flex-1 md:ml-64 mt-20 p-4 md:p-8">
        <div className="max-w-6xl mx-auto space-y-8">
          ...
          <div className="glass-panel rounded-[2rem] border border-white/10 overflow-hidden">
            ...
            {loading ? (
              <div className="p-8 text-muted text-center">Loading transactions...</div>
            ) : transactions.length === 0 ? (
              <div className="p-8 text-muted text-center">No campaign funding transactions yet.</div>
            ) : (
                ...
                <table className="w-full text-left border-collapse">
                  <thead>
                    <tr className="border-b border-white/10 bg-white/5">
                      ...
                      <th className="py-4 px-6 text-xs font-semibold text-muted uppercase tracking-wider">
                        Transaction Hash
                      </th>
                  ...
                  <tbody className="divide-y divide-white/5">
                    {transactions.map((tx) => (
                      <tr key={tx.id} className="hover:bg-white/5">
                        <td className="py-4 px-6 text-sm text-muted">{new Date(tx.date).toLocaleDateString()}</td>
                        <td className="py-4 px-6 font-medium text-white">{tx.campaign}</td>
                        <td className="py-4 px-6 text-sm font-mono text-cyan">
                          {tx.txUrl ? (
                            <a
                              href={tx.txUrl}
                              target="_blank"
                        ...
                        <td className="py-4 px-6 text-right">
                          <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-md bg-white/5 border border-white/10 text-xs font-medium text-muted">
