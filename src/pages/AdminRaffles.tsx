...
export default function AdminRaffles() {
  ...
  const raffleEnded = Boolean(raffleEndTime && raffleEndTime <= Date.now());
  const visibleCampaigns = useMemo(() => {
    return campaigns.filter((item) => {
      const endTime = getCampaignEndTime(item.end_date);
      const isPast = item.status === 'completed' || Boolean(endTime && endTime <= Date.now());
            ...
            <>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                <div className="glass-panel rounded-2xl p-5 border border-white/10">
                  ...
                  <div className="text-2xl font-semibold text-white">{stats.joined_count}</div>
                </div>
                <div className="glass-panel rounded-2xl p-5 border border-white/10">
