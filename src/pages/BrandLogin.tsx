...
export default function BrandLogin() {
  ...
  const handleTwitterLogin = async () => {
    ...
    const { error } = await supabase.auth.signInWithOAuth({
      provider: 'x',
      options: {
        redirectTo: window.location.origin + '/auth/callback/brand',
      },
    });
        ...
        >
          ...
          <div className="glass-panel rounded-[2.5rem] p-8 md:p-10 relative overflow-hidden h-full w-full bg-black/40 backdrop-blur-3xl">
            ...
            <div className="text-center mb-10 relative z-10">
              <h1 className="text-3xl font-semibold tracking-tight mb-3 text-white">Brand Sign In</h1>
              <p className="text-muted">Welcome back to SorsaMarket</p>
            </div>
            ...
            <div className="relative my-8 flex items-center relative z-10">
            ...
            <button 
              ...
              disabled={isLoggingIn}
              className="w-full py-4 rounded-full bg-white/5 border border-white/10 text-white font-medium flex items-center justify-center gap-3 hover:bg-white/10 transition-colors relative z-10 disabled:opacity-50"
            >
              <XLogo className="w-5 h-5 text-white" />
              {isLoggingIn ? 'Connecting...' : 'Continue with X'}
            </button>
            ...
            <div className="mt-8 text-center relative z-10">
              <p className="text-muted text-sm">
                ...
                <button 
                  onClick={() => navigate('/auth/brand/register')}
