...
export default function BrandRegister() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (data.user) data.session ? navigate('/brand/profiles/new') : setIsRegistered(true);
    } catch (err) {
  ...
  return (
    <div className="min-h-screen bg-black text-[#F5F5F7] font-sans flex flex-col items-center justify-center p-6 selection:bg-cyan/30">
      <div className="w-full max-w-md">
        <motion.button initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.8, ease: appleEase }} onClick={() => navigate('/auth/brand')} className="flex items-center gap-2 text-muted hover:text-white transition-colors mb-8 text-sm font-medium">
          <ArrowLeft className="w-4 h-4" /> Back to Sign In
        </motion.button>

        {isRegistered ? (
          <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 0.8, ease: appleEase }} className="glass-panel rounded-[2.5rem] p-8 md:p-12 border border-white/10 text-center space-y-6 bg-black/40 backdrop-blur-3xl">
            ...
            <p className="text-muted leading-relaxed">We've sent a confirmation link to <span className="text-white font-medium">{email}</span>.</p>
            <button onClick={() => navigate('/auth/brand')} className="px-8 py-4 rounded-full bg-white text-black font-semibold hover:scale-[1.02] transition-transform duration-300 flex items-center gap-2 mx-auto">Return to Login <ArrowRight className="w-4 h-4" /></button>
          </motion.div>
        ) : (
          <motion.div layout initial={{ opacity: 0, y: 40 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8, ease: appleEase, delay: 0.1 }} className="relative rounded-[2.5rem] p-[1px] overflow-hidden group">
            ...
            <div className="glass-panel rounded-[2.5rem] p-8 md:p-10 relative overflow-hidden h-full w-full bg-black/40 backdrop-blur-3xl border border-white/10">
              ...
              <div className="mt-8 text-center relative z-10"><p className="text-muted text-sm">Already have an account? <button onClick={() => navigate('/auth/brand')} className="text-cyan hover:underline font-medium">Sign in</button></p></div>
            </div>
          </motion.div>
        )}
      </div>
